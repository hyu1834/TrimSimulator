...
IO_Trim_Simulator::IO_Trim_Simulator(std::vector<Command*>& commands, int maxparallelops):Simulator(commands, maxparallelops)	{
	totalIOTime = 0;
	totalTrimTime = 0;
	IOqueuelength = 0;
	Trimqueuelength = 0;
...
IO_Trim_Simulator::~IO_Trim_Simulator()	{
	commandPtr = NULL;
}

void IO_Trim_Simulator::startSimulation(double readProcessTime, double writeProcessTime, double trimProcessTime)	{
	FILE* log;
	log = fopen("../../../io_trim_simulator.csv", "w");
	int count = 0;
...
void IO_Trim_Simulator::StatCollect()
{
