...
Hold_Trim_Simulator::Hold_Trim_Simulator(std::vector<Command*>& commands, int maxparallelops) :Simulator(commands, maxparallelops) {
	totalIOTime = 0;
	totalTrimTime = 0;
	IOqueuelength = 0;
	Trimqueuelength = 0;
...
Hold_Trim_Simulator::~Hold_Trim_Simulator() {
	commandPtr = NULL;
}

void Hold_Trim_Simulator::startSimulation(double readProcessTime, double writeProcessTime, double trimProcessTime) {
	FILE* log;
	log = fopen("../../../hold_trim_simulator.csv", "w");
	int count = 0;
			...
			while (!availableDriverSlot.empty() && (!trimQueue.empty() || !ioQueue.empty())) {
				...
				if (nextTrimCommand != NULL && nextIOCommand != NULL) {
					...
					if (currentServingType == ANY_COMMAND) {
						// TRIM
						if (timeDiff > 0 && nextIOCommand->getType() == WRITE_COMMAND) {
							// execute the command
...
void Hold_Trim_Simulator::StatCollect()
{
